...
export default function TaskFormDialog({ members, task, trigger }: TaskFormDialogProps) {
    ...
    // delete and recreate it instead. So the recurrence picker only appears
    // when creating a new task; the backend (UpdateTaskAction) never reads
    // these fields on edit.
                            ...
                            <CollapsibleContent className="grid gap-4 pt-2">
                                <div className="grid gap-2">
                                    ...
                                    <Select
                                        ...
                                        onValueChange={(v) => setData('recurrence_type', v as RecurrenceMode)}
                                    >
                                        <SelectTrigger>
                                            <SelectValue />
                                        </SelectTrigger>
                                    ...
                                    <div className="grid gap-4 rounded-lg border p-3">
                                        <div className="grid grid-cols-[1fr_auto_1fr] items-end gap-2">
                                            <div className="grid gap-2">
                                                ...
                                                <Select
                                                    value={data.rrule_freq}
                                                    onValueChange={(v) => setData('rrule_freq', v as RruleFreq)}
                                                >
                                                    <SelectTrigger>
                                                        <SelectValue />
                                                    </SelectTrigger>
                                        ...
                                        {data.rrule_freq === 'WEEKLY' && (
                                            <div className="grid gap-2">
                                                ...
                                                <ToggleGroup
                                                    type="multiple"
                                                    value={data.rrule_byday}
                                                    onValueChange={(v) => setData('rrule_byday', v)}
                                                >
                                                    {WEEKDAYS.map((d) => (
                                ...
                                {data.recurrence_type === 'relative' && (
                                    <div className="grid gap-2 rounded-lg border p-3">
                                        <Label htmlFor="relative_interval_days">
                                            {t('recurrence.daysAfterCompletion')}
                                        </Label>
                                        <Input
